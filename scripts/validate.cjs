...
for (const file of ['app.js', 'tracks.js', 'track-view.js']) new vm.Script(read(file), { filename: file });
for (const page of fs.readdirSync(root).filter(file => file.endsWith('.html'))) for (const match of read(page).matchAll(/(?:src|href|poster)="([^"#]+)"/g)) {
